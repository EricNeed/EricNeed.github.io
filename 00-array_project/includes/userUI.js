...
class UserUI{
    constructor(__playerID){
        this.enable_map = false;
        this.planeID = __playerID;
    ...
    drawMap(){
        ...
        let player = character_list.list[this.planeID];
        let cornerX = floor(player.primary_parts[2]/this.GRID_IG)-floor(this.ROW_COLL/2);//left corner of the map
        let cornerY = floor(player.primary_parts[3]/this.GRID_IG)-floor(this.ROW_COLL/2);//top corner

        //display the self arrow in the center of the map, with rotation
        push();
        rotateZ(player.primary_parts[8]);
        ...
        for(const other_chara of this.character_list.list){
            let chunkX = floor(other_chara.primary_parts[2]/this.GRID_IG) - cornerX;
            let chunkY = floor(other_chara.primary_parts[3]/this.GRID_IG) - cornerY;
